...
define("E_VAL_BOT_CHECK","Unknown error, you can not register");
			...
			case 'bot':
						{
							if(isset($input_value) && strlen($input_value)>0)
							{
								$bret= $this->validate_bot($input_value);
								if(false == $bret)
								{
									$default_error_message = E_VAL_BOT_CHECK;
								}
							}
							break;
						}
